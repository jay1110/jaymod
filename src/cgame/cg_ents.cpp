	...
	switch (cent->currentState.weapon) {
		...
		case WP_LANDMINE_PGAS:
			if (cent->currentState.otherEntityNum2) {
				// AXIS
				ent->hModel       = weapon->modModels[1];
			    ent->customShader = 0;
			    ent->customSkin   = weapon->missileAxisSkin;
			}
			else {
				// ALLIES
				ent->hModel       = weapon->modModels[0];
			    ent->customShader = 0;
...
void CG_Missile( centity_t *cent ) {
    entityState_t* const s1 = &cent->currentState;
    if (s1->weapon > WP_NUM_WEAPONS)
        s1->weapon = WP_NONE;

    const weaponInfo_t* const weapon = &cg_weapons[s1->weapon];
    ...
    switch (s1->weapon) {
        ...
        case WP_SATCHEL:
            if (s1->clientNum == cg.snap->ps.clientNum) {
        ...
        case WP_ARTY:
	        if (s1->otherEntityNum2 && s1->teamNum == cgs.clientinfo[ cg.clientNum ].team) {
...
//----(SA)	whoops, didn't mean to check it in with the missile flare
	...
	if (cent->currentState.eType == ET_FP_PARTS) {
		...
		switch (cent->currentState.weapon) {
			case WP_LANDMINE:
				missileTeam = (team_t)(cent->currentState.teamNum % 4);
				break;

			case WP_LANDMINE_BBETTY:
			case WP_LANDMINE_PGAS:
				missileTeam = (team_t)(cent->currentState.teamNum % 4);
			    ent.customShader = weapon->modModels[ 2 ];
				break;

			case WP_POISON_GAS:
				missileTeam = (team_t)(cent->currentState.teamNum);
				ent.customShader = weapon->modModels[ missileTeam == TEAM_ALLIES ? 0 : 1 ];
				break;
	...
	if( cent->currentState.weapon == WP_LANDMINE || cent->currentState.weapon == WP_LANDMINE_BBETTY || cent->currentState.weapon == WP_LANDMINE_PGAS ) {
		// If they are a spectator and not a shoutcaster, bail.
		if( cgs.clientinfo[ cg.clientNum ].team == TEAM_SPECTATOR && !cgs.clientinfo[cg.snap->ps.clientNum].shoutcaster ) {
			return;
		}
		...
		// Jaybird - modified for Shoutcasters
		if( cgs.clientinfo[ cg.clientNum ].team == TEAM_SPECTATOR ) {
			if(cent->currentState.teamNum < 4 && !cent->currentState.modelindex2) {
				ent.origin[2] -= 8;
				ent.oldorigin[2] -= 8;
		...
		else if(cent->currentState.teamNum < 4) {
			ent.origin[2] -= 8;
			ent.oldorigin[2] -= 8;
	...
	if ( s1->pos.trType != TR_STATIONARY ) {
		if (s1->weapon == WP_KNIFE) {
            vec3_t delta;
            VectorCopy( s1->pos.trDelta, delta );

            vec3_t angles;
            vectoangles( delta, angles );

            angles[PITCH] += cg.time;
...
//	cabinet.hModel =	cabinetInfo[type].itemmodels[0];	
	...
	if(cent->currentState.onFireStart == -9999) {
		...
		cnt = int( MAX_CABINET_TAGS * (cent->currentState.onFireStart / (float)cent->currentState.onFireEnd) );
...
//unlagged - timenudge extrapolation
	...
	if ( cent->currentState.number < MAX_CLIENTS && cent->currentState.clientNum != cg.predictedPlayerState.clientNum ) {
		...
		cent->currentState.pos.trDuration = 1000 / cvars::sv_fps.ivalue;
	}
//unlagged - timenudge extrapolation
...
static void
CG_ProcessEntity( centity_t* ent ) {
    switch ( ent->currentState.eType ) {
        ...
        case ET_CABINET_A:
            CG_Cabinet( ent, CT_AMMO );
            break;

        case ET_CABINET_H:
            CG_Cabinet( ent, CT_HEALTH );
            break;
        ...
        case ET_BULLET_VOLUME:
            CBulletVolume::render( *ent );
            break;

        case ET_ALIGNED_HIT_VOLUME:
            CHitVolume::render( *ent, false );
            break;

        case ET_ORIENTED_HIT_VOLUME:
            CHitVolume::render( *ent, true );
            break;

        default:
            if ( ent->currentState.eType < 0 || ent->currentState.eType >= ET_EVENTS )
                CG_Error( "Bad entity type: %d\n", ent->currentState.eType );
            break;
    }
}
...
qboolean CG_AddLinkedEntity( centity_t *cent, qboolean ignoreframe, int atTime ) {
	...
	if (!(sParent->eFlags & EF_PATH_LINK)) {
		if( sParent->pos.trType == TR_LINEAR_PATH ) {
			...
			pos = int(cent->backdelta * (MAX_SPLINE_SEGMENTS));
