...
public:
	int AddBot(const MessageHelper &_data)
	{
		...
		for (int clNum = 1; clNum < level.maxclients; clNum++) {
			...
			if (!clEnt || clEnt->inuse || (clEnt->client && (clEnt->client->pers.connected == CON_CONNECTED || clEnt->client->pers.connected == CON_CONNECTING))) {
				continue;
			}
			else {
				useSlot = clNum;
		...
		if(ClientConnect(connectErrMsg, num, qtrue, qtrue))
		{
